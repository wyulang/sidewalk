...
let entryPath = `${config.root}\\src\\client\\${process.argv[3]}\\`.replace(/[\\]/g, '/');
let fromFile = `${config.root}\\webpack\\${process.argv[2]}\\`.replace(/[\\]/g, '/');
...
inquirer.prompt([{
    type: 'confirm',
    name: 'create',
    message: `确认创建 ${process.argv[3]} 模块?`,
    default: true
}]).then((e) => {
    if (e.create) {
        fs.mkdir(entryPath);
        fs.copy(fromFile, entryPath, function(e) {
            console.log(chalk.yellowBright.bold(`模块创建成功 启动模块为：npm run dev ${process.argv[3]}`));
        });
    }
})