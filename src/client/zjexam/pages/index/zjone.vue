<template>
  <div class='w-all jbbc pb20'>
    ...
    <el-row class="w-1200 m-auto ">
      <!-- 客服 -->
      <div class="ar-10 kefu zi-8888 jb-fc ra-5 jb-bc pp5">
        <div class="w-130 pl10 pr10 ra-3 bc-f">
          <div class="bb-e w-all bd-color center fs-18 pt10 pb10">在线客服</div>
          <div class="flex w-all pt5 pb5 ai-c">
            <span class="iconfont iconqq mr5 ml16"></span>
            <span @click="href('http://wpa.qq.com/msgrd?v=3&amp;uin=488203917&amp;site=qq&amp;menu=yes')" class="fc-666 hand">488203917</span>
          </div>
          <div class="flex w-all pb5 ai-c">
            <span class="iconfont iconqq mr5 ml16"></span>
            <span @click="href('http://wpa.qq.com/msgrd?v=3&amp;uin=496319464&amp;site=qq&amp;menu=yes')" class="fc-666 hand">496319464</span>
          </div>
        </div>
        <div class="w-130 pl10 pr10 mt10 ra-3 bc-f">
          <div class="bb-e w-all bd-color center pt10 fs-18 pb10">留言版</div>
          <textarea v-model="question" placeholder="留下您想咨询的 问题，我们将会 尽快回复。" name="" class="w-all bd-color h-70 ra-4 mt5 mb5" id="" cols="30" rows="10"></textarea>
          <div @click="setMessage" class="w-70 pt5 hand center ml20 pb5 jb-bc ra-5 mb10">提交</div>
          <div class="h-2"></div>
        </div>
        <div v-if="kefuList.length" class="w-130 pl10 pr10 mt10 ha-100 auto ra-3 bc-f">
          <div v-for="(item, index) in kefuList" :key="index" class="flex lh-15 pt6 pb6 fd-c">
            <span>Q1{{index+1}}：{{item.question}}</span>
            <span class="mt3">回：{{item.answer ||'暂无'}}</span>
          </div>
        </div>
      </div>

      <!-- 图标菜单 -->
      <div class="w-all bc-fff flex jc-b pt25 pb25 pl45 pr45 mt20">
      ...
      <div class="w-all mt20 bc-fff pt10 flex pb10">
        <div class="w-500">
          <div class="flex ai-c jc-c">
            ...
            <div @click="getLogin({name:'school',query:{type:2}})" class="jb-fc hand  fs-15">查看完整榜单</div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      banner: [
        ...
        { url: require('../../assets/one-banner/banner-0.png'), href: 'pay' },
        { url: require('../../assets/one-banner/banner-1.png'), href: '' },
        { url: require('../../assets/one-banner/banner-2.png'), href: '' },
      ],
      nemuList: [
  ...
  methods: {
    ...
    getMessage() {
      this.getMessageBoardFind().then(res => {
        if (res.code == 20000) {
          this.kefuList = res.data.array;
        }
      })
    },
    setMessage() {
      if (this.question.length < 5) {
        this.$message.error('留言内容少于5个字');
        return;
      }
      this.getMessageBoard({ question: this.question }).then(res => {
        if (res.code == 20000) {
          this.$message.success('留言成功,请等待客服回复');
          this.question = "";
          this.getMessage();
        }
      })
