  ...
  <div :class="{'h-all hidden hi-all-no':$store.state.isHeight}" class="w-all fs-14 hi-all flex fd-c">
    <header v-if="$store.state.isHeader" class="bc-fff h-70 w-all sha-b fixed at0">
      <div class="w-1200 m-auto h-70 flex ai-c">
        <div @click="$router.push({name:'index'})" class="hand"><img src="./assets/logo.png" alt=""></div>
        <div @click="$router.push({name:'index'})" class="ml10 hand">
          <h1 class="jb-fc fs-25 fb-4">金榜题名</h1>
          <div class="fs-12 fc-111">专注服务于参加浙江省高考普通类和艺体类平行志愿志愿填报的全体考生</div>
        </div>
        ...
        <div v-if="!(this.userinfo && ['1','3'].includes(this.userinfo.userSource))" class="ml20 rel qrcode h-all flex ai-c hand">
          ...
          <div class="abs at60 al-12 sha-all centent pp7 bc-fff ra-5 hide">
            <img class="w-150 h-150" src="./assets/m_qrcode.png" alt="">
            <span>扫一扫，登录手机端</span>
          </div>
        </div>
    ...
    <footer v-if="$store.state.isFooter" class="jb-bbc">
      <div class="w-1200 m-auto flex pt20 pb20">
        <div class="flex" style="width:430px;">
          ...
          <div class="ml10">
            <div>客服电话1：400-189-0908</div>
            <div class="mt15">客服电话2：0574-86578090</div>
            <div class="mt25">工作日：8:45~12:00 13:00~17:30</div>
          </div>
        </div>
        <div class="flex" style="width:333px;">
          <i class="icon iconfont iconyoujian"></i>
          <div class="ml10">
            <div>邮箱：help@joy-read.com</div>
          </div>
        </div>
        <div class="w-300">
          <div class="flex">
            <i class="icon iconfont iconweixin"></i>
            <span class="flex-1 ml10">微信公众号：zj-xinlingxian</span>
          </div>
          <div class="flex mt30">
            <i class="icon iconfont iconqq"></i>
            <span class="flex-1 ml10">QQ群：626759772</span>
          </div>
          <div class="flex">
            <i class="w-18"></i>
            <span class="flex-1 ml10">高考2群：801384262</span>
          </div>
        </div>
        <div><img class="w-92 h-92" src="./assets/jb-qrcode.png" alt=""></div>
      </div>
    </footer>
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      link.href = require('./assets/icon/favicon1.ico');
