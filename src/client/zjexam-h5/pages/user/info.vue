<template>
  <div :class="{'isPop':isPop}" class="w-all fs-14">
    <div class="pl15 pr15">
      ...
      <div @click="changPop(1)" class="flex ai-c jc-b h-50 bb-e">
        <span class=" w-90"><i class="red">*</i>所在地区：</span>
        <span class="flex-1">{{address.join('-')}}</span>
        <span class="iconfont iconright fc-aaa fs-13"></span>
      </div>
      <div @click="changPop(2)" class="flex ai-c jc-b h-50 bb-e">
        <span class=" w-90"><i class="red">*</i>所在学校：</span>
        <span class="flex-1">{{user.schoolName}}</span>
        <span class="iconfont iconright fc-aaa fs-13"></span>
      </div>
      <div v-if="user.schoolId==-1" class="flex ai-c jc-b h-50 bb-e">
        <div class="flex ai-c w-90"><i class="red">*</i><span> 学校：</span></div>
        <input v-model="user.highschool" class="input fs-14" type="text" maxlength="11" placeholder="请输入您的学校名称">
      </div>
    ...
    <mt-popup class="w-all" v-model="isPop" position="bottom">
      <div class="h-40 flex ai-c jc-e bc-f2 mb15"><span @click="isPop=false" class="h-all w-50 flex ai-c jb-fc fs-16">完成</span></div>
      <mt-picker class="w-all fs-14" :slots="popData" @change="onValuesChange"></mt-picker>
    </mt-popup>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    onValuesChange(pk, model) {
      pk.setSlotValues(2, this.addrList.filter(v => { return v.parent == model[1] }).map(v => { return v.name }))
      if (this.pkType == 1) {
        this.address = model;
        if (model.length) {
          this.user.cityId = this.addrList.find(v => { return v.name == model[1] }).id;
          let districtId = this.addrList.find(v => { return v.name == model[2] }).id;
          this.getSchool(districtId);
          setTimeout(() => {
            this.user.districtId = districtId;
          }, 200);
        }
      } else {
        if (model.length) {
          if (model[0] == '其他学校') {
            ...
            this.user.schoolName = model[0];
          } else {
            this.user.schoolId = this.schoolList.find(v => { return v.name == model[0] }).id;
            this.user.schoolName = this.schoolList.find(v => { return v.name == model[0] }).name;
          }
        }
      }
    },
    ...
    btnSave() {
      ...
      if (this.user.schoolId == -1 && !this.user.highschool) {
        this.$toast('请输入您的学校名称');
        return
      }
    ...
    this.getUserInfo().then(res => {
      if (res.code == 20000) {
        ...
        if (res.data.highschool) {
          this.user.highschool = res.data.highschool;
          this.user.schoolId = -1;
          this.user.schoolName = "其他学校"
        }
        ...
        this.address.push('浙江');

        let cityName, districtName;
        if (res.data.cityId) {
          cityName = this.addrList.find(v => { return v.id == res.data.cityId }).name;
          this.user.cityId = res.data.cityId;
          this.address.push(cityName);
        }

        if (res.data.districtId) {
          districtName = this.addrList.find(v => { return v.id == res.data.districtId }).name;
          this.getSchool(res.data.districtId, this.user.schoolId);
          this.user.districtId = res.data.districtId;
          this.address.push(districtName);
        }
